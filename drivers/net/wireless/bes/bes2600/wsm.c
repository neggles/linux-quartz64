		...
		if (!rx.status &&
			...
			if (priv->join_status == BES2600_JOIN_STATUS_STA &&
				...
				if (is_multicast_ether_addr(hdr->addr1)) {
					...
					bes2600_info(BES2600_DBG_WSM, "[WSM] RX unicast deauth: protected=%d, pmf=%d\n",
							has_protected, priv->pmf);
					/*
...
bool wsm_flush_tx(struct bes2600_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		...
		if (timeout < 0 || wait_event_timeout(hw_priv->bh_evt_wq,
			...
			//hw_priv->bh_error = 1;
			bes2600_err(BES2600_DBG_WSM, "%s:  bh_error=1\n", __func__);
			WARN_ON(1);
			wake_up(&hw_priv->bh_wq);
			return false;
		}
...
bool wsm_vif_flush_tx(struct bes2600_vif *priv)
{
	...
	if (hw_priv->bh_error) {
		...
		if (timeout < 0 || wait_event_timeout(hw_priv->bh_evt_wq,
			...
			/* Hmmm... Not good. Frame had stuck in firmware. */
			hw_priv->bh_error = 1;
			bes2600_err(BES2600_DBG_WSM, "%s:  bh_error=1 %d\n", __func__,
